class HomeLoanBanksController < ApplicationController
  ...
  def index
    @home_loan_bank = HomeLoanBank.new
  ...
  def show
    redirect_to home_loan_banks_url
  end
  ...
  def create
    @home_loan_bank = HomeLoanBank.new(home_loan_bank_params)

    respond_to do |format|
      if @home_loan_bank.save
        format.html { redirect_to home_loan_banks_url, notice: 'Home loan bank was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @home_loan_bank.update(home_loan_bank_params)
        format.html { redirect_to home_loan_banks_url, notice: 'Home loan bank was successfully updated.' }
        format.json { render :show, status: :ok, location: @home_loan_bank }
      else
        @home_loan_banks = HomeLoanBank.all.map{ |c| c = (c.id == @home_loan_bank.id)? @home_loan_bank:c}
        @home_loan_bank = HomeLoanBank.new
        format.html { render :index }
        format.json { render json: @home_loan_bank.errors, status: :unprocessable_entity }
      end
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to home_loan_banks_url, notice: 'Home loan bank was successfully destroyed.' }
  ...
  private
    ...
    def set_home_loan_bank
      @home_loan_bank = HomeLoanBank.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def home_loan_bank_params
      params.fetch(:home_loan_bank, {})
    end
    ...
    def init_params
      ...
      @home_loan_banks = HomeLoanBank.all
    end
end
